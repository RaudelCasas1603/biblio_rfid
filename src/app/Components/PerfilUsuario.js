...
export default function PerfilUsuario({ dataMensual, historialPrestamos, generosLeidos, prestamosTotales }) {
      ...
      <div className="flex flex-col items-center w-full px-4 pt-8">
        ...
        <div className="w-full max-w-4xl mt-16 grid grid-cols-1 md:grid-cols-2 gap-6">
          <div className="bg-white rounded-xl shadow-md p-6 flex flex-col justify-center items-center min-h-[350px] w-full">
            <h2 className="text-3xl font-semibold text-gray-700 mb-2">Libros prestados</h2>
            <h2 className="text-6xl font-bold text-blue-600">{prestamosTotales}</h2>
          ...
          <div className="bg-white rounded-xl shadow-md p-6 min-h-[350px] w-full flex flex-col items-center justify-center text-center">
            <h3 className="text-lg font-semibold text-gray-700 mb-4">Préstamos mensuales</h3>
            <div className="w-full max-w-md mr-10">
              <ResponsiveContainer width="100%" height={200}>
                <LineChart data={dataMensual}>
                  ...
                  <YAxis />
                  <Tooltip />
                  <Line type="monotone" dataKey="prestamos" stroke="#2563EB" strokeWidth={3} />
                </LineChart>
              </ResponsiveContainer>
            </div>
          </div>
          ...
          <div className="bg-white rounded-xl shadow-md p-6 min-h-[350px] w-full">
            <h3 className="text-lg font-semibold text-gray-700 mb-4">Géneros leídos</h3>
        ...
        <div className="w-full max-w-8xl mt-16">
          <h2 className="text-2xl font-semibold text-gray-800 mb-6">Historial de Préstamos</h2>
            ...
            <table className="min-w-full divide-y divide-gray-200">
              ...
              <tbody className="divide-y divide-gray-200">
                {historialPrestamos.map((prestamo) => (
                  <tr key={prestamo.pk_id_prestamo}>
                    <td className="px-6 py-4 whitespace-nowrap">{prestamo.titulo}</td>
                    <td className="px-6 py-4 whitespace-nowrap">{prestamo.autor}</td>
                    <td className="px-6 py-4 whitespace-nowrap">{prestamo.genero}</td>
                    <td className="px-6 py-4 whitespace-nowrap">
                      {new Date(prestamo.fecha_prestamo).toLocaleDateString()}
                    </td>
                    <td className="px-6 py-4 whitespace-nowrap">
                      {prestamo.estatus_entrega
                        ? new Date(prestamo.fecha_entrega).toLocaleDateString()
                        : "Pendiente"}
                    </td>
                  </tr>
                ))}
